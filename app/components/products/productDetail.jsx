...
const ProductDetail = (props) => {
  ...
  const addToWhishList = async (product) => {
    if (setProceed) {
      try {
        const { data } = await axios.post(
          `${URL_LOCAL_END}/wishlist/addwishlist`,
          { _id: product._id },
          {
            headers: {
              Authorization: authToken,
  ...
  return props.product === undefined || props.product === null ? null : (
    <div>
            ...
            <div className="product-detail-title">
              ...
              <div>
                ...
                props.product.category === undefined ? (
                  <h4>
                    Catalog : <small> No Catalog</small>
                  </h4>
                ) : (
                  <h4>
                    Catalog: <small> {props.product.catalog.name}</small>
                  </h4>
                )}
                ...
                props.product.category === undefined ? (
                  <h4>
                    Category: <small> No Category</small>
                  </h4>
                ) : (
                  <h4>
                    Category: <small> {props.product.category.name}</small>
                  </h4>
                )}
              </div>
              ...
              <div className="flex items-center product-add">
                <button
                  className="border  rounded-md py-2 px-4 mr-2 font-Inter dark:text-black"
                  onClick={increaseQuantity}
                >
                  +
                </button>
                <span className="text-center w-8 font-Inter dark:text-black">
                  {productQuantity}
                </span>
                <button
                  className="border rounded-md py-2 px-4 ml-2 font-Inter dark:text-black"
              ...
              >
                <Rating
                  readonly={true}
                  size={22}
                  initialValue={Math.round(props.product.rating)}
                />
              </div>
              ...
              <div className="product-add-cart">
                ...
                >
                  <button
                    type="button"
                    className="py-2 px-4 ml-10 font-semibold tracking-wide bg-transparent text-gray-800 border border-green-300 rounded-md"
                    onClick={() => addToCart(props.product)}
                  >
                    Add To Cart{" "}
                  </button>
                  <div
                    className=" ml-8 mt-10 rounded-full bg-black p-2 text-[#E74040]"
                    onClick={() => addToWhishList(props.product)}
                  >
                    <svg
                  ...
                  <button
                    type="button"
                    className="text-white ml-5 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center me-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                  >
                    <svg
      ...
      <div className="product-tab-body">
        ...
        {props.product === undefined || props.product === null ? null : (
