...
const SearchPage = (props) => {
    ...
    const getCategories = async () => {
        ...
        // if (setProceed) {
            try {
                const { data } = await axios.get(`http://localhost:43000/api/categories`, {
                    // headers: {
    ...
    const getsSearchParams = async (search) => {
        ...
        // if (setProceed) {
            setloading(true)
            try {
                const { data } = await axios.get(`${URL_LOCAL_END}/product/index?search=${search}`, {
                    // headers: {
                    //     'Authorization': authToken
                    // }
                })
 ...
 // if (setProceed) {
           setloading(true)
            try {
                console.log('------getCatalogProduct------1-------->',search,searchData)
                const { data } = await axios.get(`${URL_LOCAL_END}/product/index?catalog=${searchData}`, {
                                              ...
                                              <ul>
                                                  {
                                                      ...
                                                      <a  href="#"  key={index} onClick={()=>ShowCatalog(item)}><li >{item.name}</li></a>
                                                  ))}
                                              </ul>
                                          </div>
                                  :   show.page === 2 ?
                                     <div className="product-category">
                                          ...
                                          <div className="product-title">
                                          <button type="button" onClick={()=>setShow({page:1,title:'home'})} class="px-3 py-2 text-xs font-medium text-center inline-flex items-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800">
                                              <svg class="w-4 h-4 text-gray-800 dark:text-white mr-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
                                              <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 5H1m0 0 4 4M1 5l4-4"/>
                                             </svg>
                                            Back
                                          </button>
                                           <h5 className="font-Inter" style={{position:"relative",top:'4px'}}> {category.name} </h5>
                                          </div>
                                          <ul>
                                              {
                                                  catelog.map((item,index) => ( 
                                                      <a href="#"  key={index} onClick={()=>getCatalogProduct(item._id)}><li >{item.name}</li></a>
                                                 ))
                                          }
                                          </ul>
                                      </div>
