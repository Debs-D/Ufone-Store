    ...
    if (setProceed) {
      try {
        ...
        const response = await axios.delete(
          `${URL_LOCAL_END}/cart/deletecart/${product._id}`,
          {
            headers: {
              Authorization: authToken,
            },
            ...
            cart.map((item, index) => (
              <div key={item._id} className="cart-product-body">
                <div className="cart-product-img">
                  <a
                    href={`/product/${ToSeoUrl(item?.productId?.name)}/${
                      item?.productId?._id
                    }`}
                  >
                ...
                <div className="cart-product-detail">
                  ...
                  >
                    <div className="cart-shape">
                      <div
                        className="h-[26px] w-[26px] rounded-full bg-gray-200 mr-4"
                        style={{ background: "#E2E4EB" }}
                      ></div>
                      <div
                        className="h-[26px] w-[26px] rounded-full bg-gray-200"
                        style={{ background: "#FF0000", opacity: "0.2" }}
                      ></div>
                    </div>
                    <h3 className="font-Inter">{item.productId.name}</h3>
                    <h4 className="font-Inter">
                      {moneyFormat(item.productId.price)}
                    </h4>
                  </a>
                  <div className="flex items-center product-add">
                    ...
                    <div className="product-rating" style={{ display: "flex" }}>
                      <Rating
                        readonly={true}
                        size={22}
                        initialValue={Math.round(item.productId.rating)}
                      />
                    </div>
                  </div>
                  <div className="product-remove">
                    ...
                    >
                      <svg
                        ...
                        aria-hidden="true"
                        xmlns="https://cdn-icons-png.freepik.com/256/6861/6861362.png?semt=ais_hybrid"
                        fill="currentColor"
                        viewBox="0 0 18 21"
                      >
